...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val grouped = lower.groupBy((ch: Char) => ch)
    grouped.map { case (c, str) => (c, str.length) }.toList.sorted
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val combined = s.mkString("")
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val xMap = x.toMap
    val subtraced = y.foldLeft(xMap) {
      case (map, (c, n)) =>
        val newCount = map(c) - n
        if (newCount > 0) map.updated(c, newCount)
        else map - c
    }
      ...
      def loop(occurrences: Occurrences): List[Sentence] = {
        if (occurrences.isEmpty) List(Nil)
          ...
          for {
            ...
            wordList <- dictionaryByOccurrences.get(comb).toList
            word <- wordList
