...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    async{
      try{
        for(part <- response if token.nonCancelled){
          exchange.write(part)
          Thread.sleep(10)
        }
      } catch {
        case e: Exception => println(s"[respond] error: ${e.getMessage}")
      } finally {
        exchange.close()
      }
    }
    ...
    async{
      while (cts.cancellationToken.nonCancelled) {
        try {
          ...
          async { respond(xchg, cts.cancellationToken, handler(req))}
        } catch {
          case _: java.net.BindException =>
            println("[start] Port already in use.")
            cts.unsubscribe()
          case _: java.util.concurrent.RejectedExecutionException =>
            println("[start] Server stopped.")
            cts.unsubscribe()
          case _: Throwable =>
        }
      }
    }
    Subscription(serverSub, cts)
  }
}
